...
void USART1_IRQHandler(void)
{
	if(USART_GetITStatus(USART_LCD,USART_IT_RXNE) != RESET)
	{
		...
		GFIFOPush(&LCDRxFifo, (uint8_t)USART_ReceiveData(USART_LCD) );
		//event delivery
	...
	switch(mode)
	{
		case UNBLOCK_TRANS_MODE:
			for(loop = 0 ; loop < size ; loop++ )
				GFIFOPush(&LCDTxFifo, *(ptr+loop) );
			USART_ITConfig(USART_LCD, USART_IT_TC , ENABLE);
...
//				if( *(ptr+loop) == '\r' )
//					blockdelay_nus(2000);
				USART_SendData(USART_LCD, *(ptr+loop) );
				while(USART_GetFlagStatus(USART_LCD, USART_FLAG_TC) == RESET );
			}
			if(*(ptr+loop-1)== '\n')
				blockdelay_nus(2000);
		break;
